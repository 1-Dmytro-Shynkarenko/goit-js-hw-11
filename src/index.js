...
// Головна функція, асинхронна, що включає в себе параметри початкового стану до введеня даних в форму і обробку результату введеня даних, звернення до функцій виведення оповіщень, кількості виведених результатів і скролу сторінки
async function formSubmit(event) {
    ...
    if (hits.length === 0) {
        ...
        // В разі наявного збігу пошуку по бекенду, спрацьовує виклик оповіщення з розрахунком кількості знайдених результатів
        alertYesImagesFound(totalHits);
        totalPages = Math.ceil(totalHits / perPage);
    }

    // Умова - за результатом розрахунку, якщо результат пошуку більший за 40 (perPage) знімається клас іs-hidden з кнопки завантаження додаткових результатів. Кнопка відоражається
...
// Функція у разі досягнення закінчення доступного для завантаження масиву даних забирає клас is-hidden з контейнеру з відповідним текстом у розмітці
